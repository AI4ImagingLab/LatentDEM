...
import emutil.utils_deblur as deblur
import os
...
def data_solution_deblur(z, x, y, alpha):
    ...
    res_fft = (Fx_sumFy + fft.fft2((alpha * z).float())) / (F2x_sum + alpha)
...
def regularization_step(k, lamb, beta, denoiser=None, how='pnp'):
    _, _, n,p = k.shape
    if how == 'l1':
        res = k - (lamb / beta)
        ...
        if not lamb ==0:
            ...
            res = k
    # elif how=='bm3d':
    #     sigma_psd = sqrt(lamb/beta)
    #     res = bm3d.bm3d(k[0,0].cpu().detach().numpy(), sigma_psd=sigma_psd)
    #     res=torch.tensor(res).to(k.device).view(k.shape)
    return normalize_kernel(res)

def HQS_ker(y, x, sigma, init_ker, denoiser=None, reg='pnp', beta=1e5, lamb=1, n_iter=100, ksize=64,index=None):##here has a ksize
    ...
    for i in range(n_iter):
        ...
        # if i==0:
        #     file_path = os.path.join('/home/chensiyi/PSLD/stable-diffusion/outputs/psld-samples-origin-mb-bm3d/samples', f"progress/k_imme_denoiser"+str(index)+"_before_.png")
        #     k_np = k0[0,0].cpu().detach().numpy()
        #     plt.imsave(file_path, k_np/k_np.max())
        z0 = regularization_step(k0, lamb, beta, denoiser=denoiser, how=reg)
        # if i==0:
        #     file_path = os.path.join('/home/chensiyi/PSLD/stable-diffusion/outputs/psld-samples-origin-mb-bm3d/samples', f"progress/k_imme_denoiser"+str(index)+"_after.png")
        #     z_np = z0[0,0].cpu().detach().numpy()
        #     plt.imsave(file_path, z_np/z_np.max())
